...
import UsersForm from './FooterLayoutForm';
import {
  GeneralPageInterface,
  GeneralPagesResponseInterface,
} from './interfaces';
import GeneralPagesDeleteAlertForm from './FooterLayoutDeleteAlertForm';

interface FilterValues {
  name: string | null;
}

function FooterLayoutApp() {
  ...
  const fetchGeneralPages = async (filteredValues: FilterValues) => {
    try {
      const response: AxiosResponse<GeneralPagesResponseInterface> =
        await axios.get(
          `${FOOTER_LAYOUT}?filter=name,${filteredValues.name ? filteredValues.name : null}&limit=${pageSize}&page=${pageNo + 1}`
        );
  ...
  const GeneralPagesRemoveHandler = async () => {
    toggleDeleteModel();
    try {
      await axios.delete(`${FOOTER_LAYOUT}/${clickedDeleteRowData.id}`);
  ...
  return (
    <div className='min-w-full max-w-[100vw]'>
      <NavigationViewComp title='Footer Layouts' />
...
export default FooterLayoutApp;
