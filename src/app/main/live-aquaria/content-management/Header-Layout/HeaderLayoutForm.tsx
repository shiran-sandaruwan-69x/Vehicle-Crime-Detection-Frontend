...
function HeaderLayoutForm(props: Props) {
  const {
    isAdd,
    className,
    isOpen,
  ...
  async function saveGeneralPage(data: GeneralPageFormData) {
    setLoading(true);
    try {
      await axios.post(HEADER_LAYOUT, data);
      toast.success('Header Layout created successfully');
    } catch (error) {
  ...
  async function updateGeneralPage(data: GeneralPageFormData) {
    setLoading(true);
    try {
      await axios.put(`${HEADER_LAYOUT}/${selectedRow.id}`, data);
      toast.success('Header Layout updated successfully');
    } catch (error) {
      const axiosError = error as ExtendedAxiosError;
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      fullWidth
                      size='small'
                      error={!!errors.header_top_color}
                      helperText={errors?.header_top_color?.message}
                      required
                      disabled={isView}
                    // inputProps={{ readOnly: true }}
                    />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      fullWidth
                      size='small'
                      error={!!errors.header_bottom_color}
                      helperText={errors?.header_bottom_color?.message}
                      required
                      disabled={isView}
                    // inputProps={{ readOnly: true }}
                    />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <FormControlLabel
                      ...
                      label={`Header Layout ${field.value === 1 ? 'Active' : 'Inactive'}`}
                    />
                  )}
                />
              </Grid>
