...
function GeneralPageForm(props: Props) {
  const {
    isAdd,
    className,
    isOpen,
  ...
  async function saveGeneralPage(data: GeneralPageFormData) {
    setLoading(true);
    try {
      await axios.post(POST_GENERAL_PAGE, data);
      toast.success('General Page created successfully');
    } catch (error) {
  ...
  async function updateGeneralPage(data: GeneralPageFormData) {
    setLoading(true);
    try {
      await axios.put(`${UPDATE_GENERAL_PAGE}${selectedRow.id}`, data);
      toast.success('General Page updated successfully');
    } catch (error) {
      const axiosError = error as ExtendedAxiosError;
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      fullWidth
                      size='small'
                      error={!!errors.meta_data}
                      helperText={errors?.meta_data?.message}
                      required
                      disabled={isView}
                      // inputProps={{ readOnly: true }}
                    />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <FormControlLabel
                      ...
                      label={`Page ${field.value === 1 ? 'Active' : 'Inactive'}`}
                    />
                  )}
                />
              </Grid>
