...
import { ReasonModifiedData } from './VideoTopicTypes';
...
function VideoTopicDeleteAlertForm({ toggleModal, isOpen, clickedRowData, handleAlertForm }: Props) {
	return (
		<Dialog
			open={isOpen}
			onClose={toggleModal}
		>
			...
			<DialogContent>
				<DialogContentText className="text-[10px] sm:text-[12px] lg:text-[14px]">
					...
					<span className="font-bold">{clickedRowData.id ? clickedRowData.id : ''}</span> this Video Topic? */}
					Are you sure you want to delete the video topic{' '}
					<span className="font-bold">{clickedRowData.id ? clickedRowData.id : ''}</span>?
				</DialogContentText>
			</DialogContent>
			<DialogActions>
				<Button
...
export default VideoTopicDeleteAlertForm;
