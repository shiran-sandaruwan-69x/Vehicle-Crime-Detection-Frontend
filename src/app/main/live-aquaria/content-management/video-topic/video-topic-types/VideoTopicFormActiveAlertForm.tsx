...
function VideoTopicActiveAlertForm({ toggleModal, isOpen, clickedRowData, handleAlertForm }: Props) {
	return (
		<Dialog
			open={isOpen}
			onClose={toggleModal}
		>
			...
			<DialogContent>
				<DialogContentText className="text-[10px] sm:text-[12px] lg:text-[14px]">
					...
					<span className="font-bold">{clickedRowData.active === true ? 'Active' : 'Inactive'}</span> this
					Video Topic?
				</DialogContentText>
			</DialogContent>
			<DialogActions>
...
export default VideoTopicActiveAlertForm;
