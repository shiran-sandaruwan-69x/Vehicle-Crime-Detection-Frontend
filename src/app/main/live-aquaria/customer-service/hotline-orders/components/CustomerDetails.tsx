...
import { CustomerDetailsByIdResponseInterface, CustomersInterface, CustomersResponseInterface } from '../interfaces';
...
function CustomerDetails({ setSeletedCustomer, seletedCustomer }: Props) {
	...
	const fetchCustomer = async () => {
		setLoading(true);
		try {
			const response: AxiosResponse<CustomersResponseInterface> = await axios.get(CUSTOMERS);
	...
	const renderAddressFields = (prefix: 'default_billing_address' | 'default_shipping_address') => (
		<Grid
			container
			spacing={2}
			className="pt-[5px]"
	...
	return loading ? (
		...
		<Formik
			initialValues={{
				...
				creditsPoints: seletedCustomer?.total_credit_points ? seletedCustomer?.total_credit_points : 0,
