...
import { addressUpdate } from '../../../../../axios/services/live-aquaria-services/customer-services/CustomerService';
import FormDropdown from '../../../../../common/FormComponents/FormDropdown';
import TextFormField from '../../../../../common/FormComponents/FormTextField';
import {
	AddressApiResponse,
	CustomerProfileAddressEditSubmitData,
	dropDown,
	TableRowData
...
interface Props {
	open: boolean;
	handleClose: () => void;
	addressData1: AddressApiResponse;
	isCounties: dropDown[];
...
function ProfileEditAddress({
	...
	const handleUpdate = async (values: CustomerProfileAddressEditSubmitData) => {
		const addressObj = {
		...
		setLoading(true);
		try {
			const addressId = addressData1?.id ?? null;
			const userId = clickedRowData.id ?? null;
			const response = await addressUpdate(userId, addressId, addressObj);
	...
	return (
		...
		>
			<DialogTitle className="flex justify-between items-center gap-[10px] pb-0">
				<Box className="text-[12px] sm:text-[14px] lg:text-[16px] text-center">{t('EDIT_MY_ADDRESS')}</Box>
				<IconButton
					edge="end"
					color="inherit"
					onClick={handleClose}
			...
			<DialogContent sx={{ overflowX: 'hidden' }}>
				<Formik
					initialValues={{
						addressLine1: addressData1?.address_line_1 || '',
						addressLine2: addressData1?.address_line_2 || '',
						addressLine3: addressData1?.address_line_3 || '',
						zipAndPostalCode: addressData1?.zip_code || '',
						city: addressData1?.city || '',
						state: addressData1?.state || '',
						country: addressData1?.country.code || '',
						defaultShipping: addressData1?.is_default_shipping === 1 ? 'yes' : 'no',
						defaultBilling: addressData1?.is_default_billing === 1 ? 'yes' : 'no'
					}}
							...
							>
								...
								>
									<FormControl component="fieldset">
										...
										>
											<FormControlLabel
												disabled={addressData1?.is_default_shipping === 1}
												value="yes"
												control={<Radio />}
												label="Yes"
											/>
								...
								>
									<FormControl component="fieldset">
										...
										>
											<FormControlLabel
												disabled={addressData1?.is_default_billing === 1}
												value="yes"
												control={<Radio />}
												label="Yes"
											/>
								...
								<Grid
									item
									xs={12}
									className="pt-[1px!important]"
								>
									<span className="text-[10px] text-gray-700 italic">
										<b className="text-red">Note : </b>
										Please add a default billing / shipping address first.
									</span>
								</Grid>
