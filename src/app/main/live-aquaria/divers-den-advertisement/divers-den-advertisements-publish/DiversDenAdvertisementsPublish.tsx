...
import {
	getAllAdvanceFilteringPendingDriversDenAdvertisementsWithPagination,
	getAllPendingDriversDenAdvertisementsWithPagination,
...
import OnApproveComp from '../divers-den-advertisements/components/OnApproveComp';
...
function DiversDenAdvertisementsPublish({ onViewClick }: Props) {
	...
	const getAllDriversDenAdvertisements = async () => {
		...
		try {
			const response: GetDiversDenAdvertisementsResponseType =
				await getAllPendingDriversDenAdvertisementsWithPagination(pageNo, pageSize);
	...
	const handleReasonAlertForm = async () => {
		...
		if (selectedRows.length !== 0) {
			const filteredIds: string[] = selectedRows
				.filter((row) => {
					if (!row.item_media) return false;

					const imageMedia = row.item_media.filter((media) => media.type === 'image');
	...
	const changePageNoOrPageSize = async (filteredValues: GeneralAdvSearchSubmitData) => {
		...
		try {
			const response: GetDiversDenAdvertisementsResponseType =
				await getAllAdvanceFilteringPendingDriversDenAdvertisementsWithPagination(
			...
			>
				...
				>
					<MaterialTableWrapper
						...
						onSelectionChange={(rows: DiversDenAdvertisementsModifiedDataResponseType[]) => {
...
export default DiversDenAdvertisementsPublish;
