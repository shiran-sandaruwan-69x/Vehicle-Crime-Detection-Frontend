...
import SneakPeekEmailAlertComp from './SneakPeekEmailAlertComp';
...
interface Props {
	...
	getAllSneakPeek: () => void;
	isMode: string;
}

function NewSneakPeekEmailComp({ toggleModal, isOpen, clickedRowData, getAllSneakPeek, isMode }: Props) {
	...
	const getAllDriversDenAdsById = async () => {
		...
		try {
			const response: AxiosResponse<SneakPeekApiResById> = await axios.get(`${CREATE_SNEAK_PEEK}/${id}`);
	...
	const getAllDriversDenAds = async () => {
		...
		try {
			const response: AxiosResponse<GetDiversDenAdvertisementsResponseType> = await axios.get(
				`${GET_DIVERS_DEN}?limit=${pageSize}&page=${pageNo}`
			);
	...
	const getScheduleTime = async () => {
		try {
			const response: AxiosResponse<ScheduleTimeApiRes> = await axios.get(`${GET_SCHEDULE_TIME}`);
	...
	const handleAlertOkForm = async () => {
		...
		if (isMode === 'create') {
			try {
				await axios.post(`${CREATE_SNEAK_PEEK}`, data);
				getAllSneakPeek();
			...
			const id = clickedRowData?.id ?? null;
			try {
				await axios.put(`${CREATE_SNEAK_PEEK}/${id}`, data);
				getAllSneakPeek();
...
export default NewSneakPeekEmailComp;
