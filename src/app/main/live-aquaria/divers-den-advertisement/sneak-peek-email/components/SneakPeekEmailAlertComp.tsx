import Dialog from '@mui/material/Dialog';
import DialogTitle from '@mui/material/DialogTitle';
import ReportIcon from '@mui/icons-material/Report';
import DialogContent from '@mui/material/DialogContent';
import { DialogContentText } from '@mui/material';
import DialogActions from '@mui/material/DialogActions';
import Button from '@mui/material/Button';
import React from 'react';
import {ScheduleTimeRes} from "../sneak-peek-email-type/SneakPeekEmailType";

interface Props {
	toggleModal: () => void;
	isOpen: boolean;
	scheduleTime: ScheduleTimeRes;
	handleAlertForm: () => void;
}

function SneakPeekEmailAlertComp({ toggleModal, isOpen, scheduleTime, handleAlertForm }: Props) {
	return (
		<Dialog
			open={isOpen}
			onClose={toggleModal}
		>
			<DialogTitle className="flex items-center gap-[5px] text-[16px] font-bold">
				<ReportIcon className="text-red text-[20px]" />
				Are you sure?
			</DialogTitle>
			<DialogContent>
				{scheduleTime?.is_active === 1 ? (
					<DialogContentText className="text-[10px] sm:text-[12px] lg:text-[14px] text-center">
						<span className="font-bold">Scheduled Time : {scheduleTime?.value || ''}</span>
						<div className="mt-4">All the New Sneak Peek Emails Will be Sent Based on the Scheduled Time</div>
					</DialogContentText>
				) : (
					<DialogContentText className="text-[10px] sm:text-[12px] lg:text-[14px]">
						<div>All the New Sneak Peek Emails Will be Sent Once You Click "OK" </div>
					</DialogContentText>
				)}
			</DialogContent>
			<DialogActions>
				<Button
					className="flex justify-center items-center min-w-[100px] min-h-[36px] max-h-[36px] text-[10px] sm:text-[12px] lg:text-[14px] text-white font-500 py-0 rounded-[6px] bg-primaryBlue hover:bg-primaryBlue/80 bokShadow"
					variant="contained"
					size="medium"
					onClick={handleAlertForm}
				>
					OK
				</Button>
				<Button
					className="flex justify-center items-center min-w-[100px] min-h-[36px] max-h-[36px] text-[10px] sm:text-[12px] lg:text-[14px] text-gray-600 font-500 py-0 rounded-[6px] bg-gray-300 hover:bg-gray-300/80 boxShadow"
					onClick={toggleModal}
				>
					NO
				</Button>
			</DialogActions>
		</Dialog>
	);
}

export default SneakPeekEmailAlertComp;
