...
function NewCodeGenerator({ isOpen, toggleModal, fetchAllGeneratedCodeValues }: NewCodeGeneratorProps) {
	...
	const createNewCode = async (values: FormValues) => {
		setDataLoading(true);
		try {
			const data = {
			...
			await createNewCodeFormat(data).then(() => toast.success('Created successfully'));
	...
	return (
		...
		>
			<DialogTitle className="pb-0">
				<h6 className="text-[10px] sm:text-[12px] lg:text-[14px] text-gray-600 font-400">Generate Code</h6>
			</DialogTitle>
			<DialogContent>
				<Formik<FormValues>
					initialValues={{
...
export default NewCodeGenerator;
