...
function BoxChargeActiveAlertForm({ toggleModal, isOpen, clickedRowData, handleAlertForm }: Props) {
	return (
		<Dialog
			open={isOpen}
			onClose={toggleModal}
		>
			...
			<DialogContent>
				<DialogContentText className="text-[10px] sm:text-[12px] lg:text-[14px]">
					...
					<span className="font-bold">{clickedRowData.active === true ? 'Inactive' : 'Active'}</span> this Packing Material Charge?
				</DialogContentText>
			</DialogContent>
			<DialogActions>
				<Button
...
export default BoxChargeActiveAlertForm;
