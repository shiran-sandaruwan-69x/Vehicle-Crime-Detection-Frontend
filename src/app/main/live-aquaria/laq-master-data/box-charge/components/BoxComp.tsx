...
import { BoxSubmitCalData, BoxSubmitData, ModifiedPackingMaterialData } from '../box-charge-types/BoxChargeTypes';
import {
	createBoxCharge,
	updateBoxCharge
...
function BoxComp({ clickedRowData, compType, getBoxCharge, toggleModal }: Props) {
	...
	const schema = yup.object().shape({
		...
		boxChargeName: yup.string().required('Box charge name is required'),
		boxCharge: yup
			.number()
	...
	const handleClearForm = (resetForm: FormikHelpers<BoxSubmitData>['resetForm']) => {
		resetForm({
			values: {
				length: '',
				width: '',
	...
	const changeWidth = async (value, form: FormikProps<BoxSubmitCalData>) => {
	...
	const changeLength = async (value, form: FormikProps<BoxSubmitCalData>) => {
	...
	const changeHeight = async (value, form: FormikProps<BoxSubmitCalData>) => {
	...
	const onSubmit = async (values: BoxSubmitData) => {
		...
		const data = {
			...
			name: values.boxChargeName,
			charge: values.boxCharge,
			diameter: null,
		...
		<div className="min-w-full max-w-[100vw]">
			<Formik
				initialValues={{
					...
					boxChargeName: clickedRowData.name ? clickedRowData.name : '',
					boxCharge: clickedRowData.charge ? clickedRowData.charge : ''
				}}
