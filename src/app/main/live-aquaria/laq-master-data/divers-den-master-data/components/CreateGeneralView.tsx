...
import { CreateGeneralViewResetFormTypes, DriversDenCisCodeDataType } from '../drivers-den-types/DriversDenTypes';
...
interface Props {
	...
	toggleModal?: () => void;
	setCisCodeData: React.Dispatch<React.SetStateAction<DriversDenCisCodeDataType>>;
}

function CreateGeneralView({ isTableMode, onNext, initialValues, isCisCodeData, setCisCodeData, toggleModal }: Props) {
	...
	const getAllParentCategory = async () => {
		try {
			...
			const firstIndexData: Category[] = [data1[0]];
			setAllData(firstIndexData);
			setParentCategory1(
	...
	return (
		<div className="min-w-full max-w-[100vw]">
			<Formik
				enableReinitialize
				initialValues={{
					cisCode: isCisCodeData.cis_code || '',
					commonName: isCisCodeData.common_name || '',
					scientific_name1: isCisCodeData.scientific_name || '',
					description: isCisCodeData.description || '',
					gender: isCisCodeData.gender || '',
					size: isCisCodeData.size || '',
					age: isCisCodeData.age || '',
					origins: isCisCodeData.origins || '',
					length: isCisCodeData.length || '',

					product_name: initialValues.product_name || isCisCodeData.common_name || '',
					scientific_name: initialValues.scientific_name || isCisCodeData.scientific_name || '',
				...
				onSubmit={(values: CreateGeneralViewTypes) => {
					onNext(values);
				}}
			>
				{({ values, setFieldValue, isValid, resetForm, touched, errors }) => (
					<Form>
						{Object.keys(isCisCodeData).length === 0 ? null : (
							<Grid
								container
								spacing={2}
