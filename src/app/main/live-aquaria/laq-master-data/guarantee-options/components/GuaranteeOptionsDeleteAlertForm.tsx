...
import { ModifiedCategoryDataType } from '../../category-nanagement/category-nanagement-types/CategoryManagementTypes';

interface Props {
	toggleModal: () => void;
	isOpen: boolean;
	clickedRowData: ModifiedCategoryDataType;
	handleAlertForm: () => void;
}

function GuaranteeOptionsDeleteAlertForm({ toggleModal, isOpen, clickedRowData, handleAlertForm }: Props) {
	return (
		<Dialog
			open={isOpen}
			onClose={toggleModal}
		>
			...
			<DialogContent>
				<DialogContentText className="text-[10px] sm:text-[12px] lg:text-[14px]">
					Are you sure you want to delete this Guarantee Option?
				</DialogContentText>
			</DialogContent>
			<DialogActions>
				<Button
...
export default GuaranteeOptionsDeleteAlertForm;
