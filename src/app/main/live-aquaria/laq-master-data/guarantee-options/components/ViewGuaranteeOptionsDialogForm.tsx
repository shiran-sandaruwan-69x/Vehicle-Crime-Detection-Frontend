...
import { updateGuaranteeOptions } from '../../../../../axios/services/live-aquaria-services/master-data-services/MasterDataServices';
import { GuaranteeOptions, ModifiedGuaranteeOptionsDataType } from '../guarantee-options-types/GuaranteeOptions';
...
interface Props {
	toggleModal: () => void;
	isOpen: boolean;
	getAllGuaranteeOptions: () => void;
	clickedRowData: ModifiedGuaranteeOptionsDataType;
	compType: string;
}

function ViewGuaranteeOptionsDialogForm({
	toggleModal,
	isOpen,
	...
	const [isNewGuaranteeOptionsDataLoading, setNewGuaranteeOptionsDataLoading] = useState(false);
	...
	const onSubmit = async (values: GuaranteeOptions) => {
		const id: string = clickedRowData.id ? clickedRowData.id : '';
		const image = images.length > 0 && images[0].base64 ? images[0].base64 : null;
		const data = {
			name: values.guaranteeName,
			description: values.description,
			attachment: image,
			is_active: 1
		};

		setNewGuaranteeOptionsDataLoading(true);
		try {
			const response = await updateGuaranteeOptions(data, id);
			toast.success('Updated Successfully');
			getAllGuaranteeOptions();
			toggleModal();
			setNewGuaranteeOptionsDataLoading(false);
		} catch (error) {
			setNewGuaranteeOptionsDataLoading(false);
	...
	const handleClearForm = (resetForm: FormikHelpers<GuaranteeOptions>['resetForm']) => {
		resetForm({
			values: {
				guaranteeName: '',
				description: ''
		...
		>
			<DialogTitle className="pb-0">
				<h6 className="text-[10px] sm:text-[12px] lg:text-[14px] text-gray-600 font-400">
					{compType === 'view' ? 'View' : 'Update'} Guarantee Option
				</h6>
			</DialogTitle>
			<DialogContent>
				<Formik
					initialValues={{
						guaranteeName: clickedRowData?.guaranteeName ? clickedRowData?.guaranteeName : '',
						...
						<Form>
							...
							>
								...
								<Grid
									...
									className="formikFormField pt-[5px!important]"
								>
									<Typography className="formTypography">Description</Typography>
									<Field
										disabled={compType === 'view'}
										name="description"
										placeholder=""
								...
								<Grid
									...
									className="formikFormField pt-[5px!important]"
								>
									<Typography className="formTypography">Attachment Image</Typography>
								...
								>
									{compType === 'view' ? null : (
										...
										>
											Update
											{isNewGuaranteeOptionsDataLoading ? (
												<CircularProgress
...
export default ViewGuaranteeOptionsDialogForm;
