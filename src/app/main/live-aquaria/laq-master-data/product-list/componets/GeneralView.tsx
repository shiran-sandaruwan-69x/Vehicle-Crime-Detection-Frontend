...
function GeneralView({ clickedRowData, isTableMode, fetchAllProductList, toggleModal }: Props) {
	const { t } = useTranslation('productList');

	const [isVProduct, setVProduct] = useState<GeneralViewModifyData>({});
	const [isGeneralViewProduct, setGeneralViewProduct] = useState<AllProductSelectionTypes>({});
	...
	const onSubmit = async (values: CreateGeneralViewTypes) => {
		...
		try {
			const response = await updateItems(reqData, id);
	...
	if (isLoading) {
		return (
			<div className="flex justify-center items-center text-primaryBlueLight">
		...
		<div className="min-w-full max-w-[100vw]">
			<Formik
				...
				initialValues={{
					...
					title: isGeneralViewProduct.title || '',
					aquaticType: isGeneralViewProduct.aquatic_type || '',
