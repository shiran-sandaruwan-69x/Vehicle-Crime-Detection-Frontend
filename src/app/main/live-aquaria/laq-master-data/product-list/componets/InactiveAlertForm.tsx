...
function InactiveAlertForm({ toggleModal, isOpen, clickedRowData, handleAlertForm }: Props) {
	return (
		<Dialog
			fullWidth
			open={isOpen}
		...
		>
			...
			<DialogContent>
				<DialogContentText className="text-[10px] sm:text-[12px] lg:text-[14px]">
					Are you sure <span>{clickedRowData || 'you want to delete this row'}</span>?
				</DialogContentText>
			</DialogContent>
			<DialogActions className="flex justify-end items-center gap-[10px]">
				<Button
...
export default InactiveAlertForm;
