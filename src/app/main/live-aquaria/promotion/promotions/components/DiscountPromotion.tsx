...
} from "../../promotion-cycle/promotion-cycle-type/PromotionCycleType";
import { PromotionDropDownData } from "../../terms-conditions/terms-conditions-types/TermsAndConditionsType";
import {
...
function DiscountPromotion({
  ...
  const [isPromotionCycle, setPromotionCycle] = useState<
  ...
  const getAllPromotionCycle = async () => {
    try {
      const response: AxiosResponse<PromotionCycleTypeApiRes> = await axios.get(
        `${CREATE_PROMOTION_CYCLE}?paginate=false`
      );
      const dropData: PromotionDropDownData[] = response?.data?.data?.map(
        (item: PromotionCycleRes) => ({
          label: item.name,
          value: item.id,
        })
      );
      setPromotionCycle(dropData);
    } catch (error) {
    ...
    if (isMode === "edit") {
      ...
      try {
        const data = {
          ...
          promotion_cycle_id: values.promotionCycle ?? null,
    ...
    <div className="discount-promotion">
      ...
      <Formik
        initialValues={{
          ...
          promotionCycle: clickedRowData?.promotion_cycle?.id || "",
              ...
              <Grid container spacing={2} className="pt-[10px]">
                ...
                >
                  ...
                  <FormDropdown
                    ...
                    id="promotionCycle"
                    placeholder=""
                    optionsValues={isPromotionCycle}
                    disabled={isMode === "view"}
                  />
                </Grid>
                <Grid
