...
interface Props {
	...
	shippingStateData: ShippingScheduleTypeDrp[];
	fetchAllAdditionalCostState: () => void;
	isTableMode: string;
}

function EditAdditionalCostStateTableData({
	isOpen,
	toggleModal,
	clickedRowData,
	shippingStateData,
	...
	const onSubmit = async (values: AdditionalCostStateSubmitData) => {
		...
		>
			<DialogTitle className="pb-0">
				<h6 className="text-[10px] sm:text-[12px] lg:text-[14px] text-gray-600 font-400">
					{isTableMode === 'edit' ? 'Edit' : 'View'} Additional Cost State
				</h6>
			</DialogTitle>
			<DialogContent>
				<Formik
					initialValues={{
						state: clickedRowData?.state?.id || '',
...
export default EditAdditionalCostStateTableData;
