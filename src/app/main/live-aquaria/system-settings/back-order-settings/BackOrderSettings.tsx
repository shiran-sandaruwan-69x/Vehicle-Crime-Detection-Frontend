...
import { SYSTEM_SETTINGS_BACK_ORDER } from 'src/app/axios/services/AdminServices';
...
export interface BackOrderSettingsResponseInterface {
	id: number;
	label: string;
	category: string;
	value: string;
...
function BackOrderSettings() {
	// const [pageNo, setPageNo] = useState<number>(0);
	// const [pageSize, setPageSize] = useState<number>(5);
	// const [count] = useState<number>(100);
	const [loading, setLoading] = useState<boolean>(true);
	const [systemSettings, setSystemSettings] = useState<BackOrderSettingsResponseInterface>(null);
	...
	const fetchFreeShippingsSettings = async () => {
		...
		try {
			const response: AxiosResponse<{
				data: BackOrderSettingsResponseInterface;
			}> = await axios.get(`${SYSTEM_SETTINGS_BACK_ORDER}`);
	...
	const handleUpdateSystemSettings = async (values: FormValues) => {
		...
		try {
			...
			const response: AxiosResponse<{
				data: BackOrderSettingsResponseInterface;
			}> = await axios.put(`${SYSTEM_SETTINGS_BACK_ORDER}`, to_update_data);
			setSystemSettings(response.data.data);
			toast.success('Back Orders settings updated successfully');
		} catch (error) {
			const axiosError = error as ExtendedAxiosError;
	...
	return loading ? (
		...
		<div className="min-w-full max-w-[100vw]">
			<NavigationViewComp title="System Settings/Back Order Settings" />

			<Formik
				initialValues={{
					days: systemSettings?.value,
				...
				{({ setFieldValue, values }: FormikProps<FormValues>) => (
					<Form>
						...
						>
							...
							>
								<Typography className="formTypography">Order Expiry Period (Days):</Typography>
								<CustomFormTextField
									name="days"
									id="days"
									type="number"
								...
								<Typography className="formTypography text-[10px] text-gray-700 italic">
									<b className="text-red">Note: </b>(Update the order expiry period by entering the
									number of days here.)
								</Typography>
							</Grid>
							...
							<Grid
								...
								className="formikFormField pt-[5px!important]"
							>
								<Typography className="formTypography">Enable Backorder</Typography>
...
export default BackOrderSettings;
