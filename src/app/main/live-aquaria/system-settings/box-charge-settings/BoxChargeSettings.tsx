...
import { SYSTEM_SETTINGS_BOX_CHARGE } from 'src/app/axios/services/AdminServices';
...
export interface BoxChargeSettingsResponseInterface {
	id: number;
	label: string;
	category: string;
	value: string;
...
function BoxChargeSettings() {
	// const [pageNo, setPageNo] = useState<number>(0);
	// const [pageSize, setPageSize] = useState<number>(5);
	// const [count] = useState<number>(100);
	const [loading, setLoading] = useState<boolean>(true);
	const [systemSettings, setSystemSettings] = useState<BoxChargeSettingsResponseInterface>(null);
	...
	const fetchFreeShippingsSettings = async () => {
		...
		try {
			const response: AxiosResponse<{
				data: BoxChargeSettingsResponseInterface;
			}> = await axios.get(`${SYSTEM_SETTINGS_BOX_CHARGE}`);
	...
	const handleUpdateSystemSettings = async (values: FormValues) => {
		...
		try {
			...
			const response: AxiosResponse<{
				data: BoxChargeSettingsResponseInterface;
			}> = await axios.put(`${SYSTEM_SETTINGS_BOX_CHARGE}`, to_update_data);
			setSystemSettings(response.data.data);
			toast.success('Box Charge Maintenance settings updated successfully');
		} catch (error) {
			const axiosError = error as ExtendedAxiosError;
	...
	const schema = yup.object().shape({
		treshold: yup
			.number()
			.required('Box Charge is required')
			.typeError('Box Charge must be a number')
			.positive('Box Charge must be a positive number')
	});
	...
	return loading ? (
		...
		<div className="min-w-full max-w-[100vw]">
			<NavigationViewComp title="System Settings/Box Charge Settings" />
				...
				{({ setFieldValue, values }: FormikProps<FormValues>) => (
					<Form>
						...
						>
							...
							>
								<Typography className="formTypography">Box Charge Maintenance</Typography>
								<CustomFormTextField
									name="treshold"
									id="treshold"
									type="number"
								...
								<Typography className="formTypography text-[10px] text-gray-700 italic">
									<b className="text-red">Note: </b>(Update the Box charge maintenance amount.)
								</Typography>
							</Grid>
							...
							>
								...
								<div className="flex justify-between items-center gap-x-[15px] sm:gap-x-[30px] w-full">
									<div className="flex items-center gap-[10px] w-full max-w-max">
										...
										/>
										{/* {systemSettings?.is_active ? 'Enabled' : 'Disabled'} */}
										<p
...
export default BoxChargeSettings;
