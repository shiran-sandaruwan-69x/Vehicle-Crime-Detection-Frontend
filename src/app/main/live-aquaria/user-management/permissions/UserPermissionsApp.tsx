...
function UserPermissionsApp() {
	...
	async function filterUserRoles() {
		setLoading(true);
		try {
	...
	const filterPermissionsForRole = async (roleId: number) => {
		try {
			const response: AxiosResponse<UserPermissions> = await axios.get(
				`${GET_PERMISSIONS_BY_ID}/${roleId}/permissions`
			);
			...
			Object.entries(response.data).forEach(([key, value]) => {
				...
				Object.entries(value).forEach(([subKey, subValue]) => {
					...
					permissionModified.push({
						id: idCounter++,
						name: subKey,
						index: subValue.find((item) => item.name === 'READ') || null,
						store: subValue.find((item) => item.name === 'CREATE') || null,
						show: subValue.find((item) => item.name === 'READ') || null,
						update: subValue.find((item) => item.name === 'UPDATE') || null,
						destroy: subValue.find((item) => item.name === 'DELETE') || null,
						parentId,
						type: 'permission'
					});
				});
	...
	const updatePermissions = async () => {
		const selectedPermissionsIds: number[] = [];

		permissions.forEach((permission: any) => {
			['index', 'show', 'store', 'update', 'destroy'].forEach((key) => {
				if (permission[key] && permission[key].action === true) {
					selectedPermissionsIds.push(permission[key].id);
				}
			});
		});
		...
		try {
			await axios.put(`${GET_PERMISSIONS_BY_ID}/${selectedRoleId}/permissions`, {
