...
function UsersForm(props: Props) {
	...
	const defaultValues = {
		...
		first_name: selectedRow ? selectedRow.name : '',
		//last_name: selectedRow ? selectedRow?.last_name : '',
		user_name: selectedRow ? selectedRow.username : '',
		...
		role_id: selectedRow
			? selectedRow.roles.map((role) => role.id)
			: [],
		nic: selectedRow ? selectedRow.nicNo : ''
	};
	...
	async function saveRole(data: any) {
		...
		try {
			const data_save = {
				...
				nicNo: data?.nic,
				roles: data?.role_id?.map((id: number) => ({ id })),
